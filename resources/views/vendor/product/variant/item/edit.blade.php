                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-header-action">
                                <form method="POST" action="{{ route('vendor.item.destroy', $variant_item) }}">
                                    @csrf
                                    @method('DELETE')

                                    <a href="{{ route('vendor.item.create', ['vid' => $variant_item->product_variant_id]) }}" class="btn btn-success"><i class="fa fa-plus"></i> Create variant item</a>
                                    <a href="{{ route('vendor.item.index', ['vid' => $variant_item->product_variant_id]) }}" class="btn btn-primary"><i class="fa fa-reply"></i> Back to variant items</a>
                                    <a href="{{ route('vendor.product.edit', $variant_item->product) }}" class="btn btn-primary"><i class="fa fa-reply"></i> Back to product</a>
                                    <a href="{{ route('vendor.item.destroy', $variant_item) }}" onclick="event.preventDefault();this.closest('form').submit();" class="btn btn-danger"><i class="fa fa-trash"></i> Delete variant item</a>
                                </form>
                            </div>
                        </div>

                        <form method="post" action="{{ route('vendor.item.update', $variant_item) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query) {
                $buttons = [
                    'edit' => '<a href="'.route(userRole().'.item.edit', $query).'" class="btn btn-warning btn-sm ml-1"><i class="fa fa-pencil-alt"></i></a>',
                    'delete' => '<a href="'.route(userRole().'.item.destroy', $query).'" class="btn btn-danger btn-sm ml-1 delete-item" data-table="productvariantitem-table"><i class="fa fa-trash"></i></a>'
                ];
